...
{
    public static class SnowflakeDurableConnectorAdf
    {
            ...
            ILogger log)
        {
            ...
            string instanceId = await starter.StartNewAsync<object>("SnowflakeDurableConnectorAdf", null, requestBodyString);

            log.LogInformation($"Started orchestration with ID = '{instanceId}'.");

            return starter.CreateCheckStatusResponse(req, instanceId, true); //true = returnInternalServerErrorOnFailure
        }


        [FunctionName("SnowflakeDurableConnectorAdf")]
        public static async Task<object> RunOrchestrator(
            [OrchestrationTrigger] IDurableOrchestrationContext context)
        {
            string requestBodyString  = context.GetInput<string>();

            List<string> reponse = await context.CallActivityAsync<List<string>>("SnowflakeDurableConnectorAdf_RunJob", requestBodyString);
            return reponse;
        }

        [FunctionName("SnowflakeDurableConnectorAdf_RunJob")]
        public static List<string> RunJob([ActivityTrigger] string requestBodyString, ILogger log)
        {
        ...
        private static List<string> runSnowflakeSqlCommands(ILogger log, string snowflakeConnectionString, string setVariableCommand, string[] sqlCommands)
        {
